"""Multi-agent supervisor for coordinating research across multiple
specialized agents.
...
def get_notes_from_tool_calls(messages: list[BaseMessage]) -> list[str]:
    ...
    return [
        tool_msg.content
        for tool_msg in filter_messages(
            messages,
            include_types="tool",
        )
    ]
...
async def supervisor(
    ...
    max_concurrent_researchers = config.get(CONFIGURABLE).get(MAX_CONCURRENT_RESEARCHERS, {})
    max_researcher_iterations = config.get(CONFIGURABLE).get(MAX_RESEARCHER_ITERATIONS, {})
