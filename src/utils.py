...
def get_current_dir() -> Path:
    ...
    This function is compatible with Jupyter notebooks
    and regular Python scripts.
...
def tavily_search_multiple(
    ...
    # Note: You can use AsyncTavilyClient to parallelize this step.
    search_docs = []
...
def summarize_webpage_content(
    ...
    try:
        ...
        model_class = config.get(CONFIGURABLE).get(MODEL_CLASS, {})
        summarization_model = model_class(**model_config)
    ...
    for url, result in unique_results.items():
        ...
        if not result.get("raw_content"):
            ...
            content = summarize_webpage_content(
                result["raw_content"][:max_content_length], config
            )
...
def tavily_search(
    ...
    summarized_results = process_search_results(unique_results, config)
...
def refine_draft_report(
    ...
    Args:
        ...
        config: RunnableConfig containing information about the large language model
