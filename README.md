...
Copy `.env.example` to `.env` and use your credentials.
...
docker compose exec thinkdepthai_deepresearch_dev /bin/bash
source /opt/venv/bin/activate
streamlit run app.py
```

or with ssh (vscode remote explorer for debugging)

```bash
docker compose -f dev-compose.yml up -d
# use vscode extension for connection here
source /opt/venv/bin/activate
streamlit run app.py
```
